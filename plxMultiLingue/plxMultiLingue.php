<?php
/**
 * Plugin plxMultiLingue
 *
 * @author	Stephane F
 *
 **/

class plxMultiLingue extends plxPlugin {
	...
	public function onActivate() {
		if(file_exists(PLX_PLUGINS.'plxMultiLingue/update')) chmod(PLX_PLUGINS.'plxMultiLingue/update',0644); # en attendant la modif en natif dans class.plx.plugins.php
		# Mise en cache du css partie administration
		$src_cssfile = PLX_PLUGINS.'plxMultiLingue/css/admin.css';
		$dst_cssfile = PLX_ROOT.PLX_CONFIG_PATH.'plugins/plxMultiLingue.admin.css';
		plxUtils::write(file_get_contents($src_cssfile), $dst_cssfile);
		# Mise en cache du ccs partie visiteurs
		$src_cssfile = PLX_PLUGINS.'plxMultiLingue/css/site.css';
		$dst_cssfile = PLX_ROOT.PLX_CONFIG_PATH.'plugins/plxMultiLingue.site.css';
		plxUtils::write(file_get_contents($src_cssfile), $dst_cssfile);
	...
	public function ConstructLoadPlugins() {
		...
		if(file_exists(PLX_ROOT.PLX_CONFIG_PATH."plugins/plxMultiLingue.xml")) {
			echo '<?php
						...
						while($file = readdir($folder)) {
							if(preg_match("/^".$ident."(.*).xml$/", $file)) {
								...
								if($uniqart["date"] <= date("YmdHi")) {
									...
									$this->infos_arts[$lang]["img"] = "<img class=\"lang\" src=\"".$this->urlRewrite(PLX_PLUGINS."plxMultiLingue/img/".$lang.".png")."\" alt=\"".$lang."\" />";
							...
							for($i=0;$i<$nb;$i++) {
								...
								if($number==$id) {
									...
									if($active) {
										...
										$this->infos_statics[$lang]["img"] = "<img class=\"lang\" src=\"".$this->urlRewrite(PLX_PLUGINS."plxMultiLingue/img/".$lang.".png")."\" alt=\"".$lang."\" />";
	...
	public function plxAdminEditConfiguration() {
		...
		if(preg_match("/parametres_base/",basename($_SERVER["SCRIPT_NAME"]))) {
			...
			$plugin = $this->plxPlugins->aPlugins["plxMultiLingue"];
			...
			# affichage sous forme de drapeaux ou de texte
				...
				foreach($this->aLangs as $lang) {
					...
					if($this->getParam('display')=='flag') {
						$img = '<img class="lang'.$sel.'" src="'.PLX_PLUGINS.'plxMultiLingue/img/'.$lang.'.png" alt="'.$lang.'" />';
	...
	public function AdminArticleContent() {
		...
		if($this->aLangs) {
			...
			foreach($this->aLangs as $lang) {
				if($this->lang!=$lang) {
					echo '<?php
					$img = "<img src=\"'.PLX_PLUGINS.'plxMultiLingue/img/'.$lang.'.png\" alt=\"'.$lang.'\" />";
					$id = $titre = "";
