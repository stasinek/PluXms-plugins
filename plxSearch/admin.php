...
$filename = PLX_ROOT.PLX_CONFIG_PATH.'plugins/plxSearch.data.php';
...
if(isset($_GET['action']) AND $_GET['action']=='resetsearchstats') {
	...
	header('Location: plugin.php?p=plxSearch');
	exit;
}
...
<?php
	...
	if($fileexists)
		echo '<p><a href="plugin.php?p=plxSearch&amp;action=resetsearchstats&amp;key='.$plxAdmin->aConf['clef'].'">'.$plxPlugin->getLang('L_RESET_STATS').'</a></p>';
?>