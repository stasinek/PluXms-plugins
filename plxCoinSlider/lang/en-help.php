...
- cocher le plugin CoinSlider et dans le déroulant "Pour la sélection", sélectionner le menu "Activer"

Configuration du diaporama CoinSlider
- Aller dans le menu Paramètres > Plugins, et cliquer sur le lien Configuration du plugin CoinSlider

Pour ajouter des images au diaporama
- aller dans le gestionnaire des médias
- cocher les images à ajouter dans le diaporama
- dans le déroulant "Pour la sélection", sélectionner le menu CoinSlider > Ajouter au diaporama

Activation et personnalisation des images du diaporama
- allez dans le menu CoinSlider (dans le bandeau des menus de l'administration)
...
<?php echo plxUtils::strCheck('<?php eval($plxShow->callHook("CoinSlider")) ?>') ?>
</div>
...
<?php
global $plxShow;
eval($plxShow->callHook("CoinSlider"));
?>
');
?>
</div>
