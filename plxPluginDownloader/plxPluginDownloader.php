<?php
/**
 * Plugin plxPluginDownloader
 *
 * @author	Stephane F
 **/

class plxPluginDownloader extends plxPlugin {
	...
	public function AdminTopEndHead() {
		echo '<link rel="stylesheet" type="text/css" href="'.PLX_PLUGINS.'plxPluginDownloader/css/style.css" />'."\n";
	}
	...
	public function AdminTopBottom() {
		...
		$string = '
			...
			$test1 = plxPluginDownloader::is_cURL();
	...
	public static function downloadRemoteFile($remotefile, $destination,$VerifyPeer=false,$VerifyHost=true) {
		if($fp = fopen($destination, 'w')) {
			...
			curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, false);
			curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
			plxPluginDownloader::curl_redir_exec($curl);
			curl_close($curl);
			fclose($fp);
		}
		else return false;
	...
	public static function curl_redir_exec(/*resource*/ $ch, /*int*/ &$maxredirect = null) {
		...
		if (ini_get('open_basedir') == '' && !plxPluginDownloader::ini_get_boolean('safe_mode')) {
