...
if(!empty($_POST)) {
	...
	foreach($_POST['button'] as $plugName => $dummy) {
		...
		if(!plxPluginDownloader::is_cURL()) {
		...
		if(!$repo = plxPluginDownloader::getRepository($cache_dir.$repository_xmlfile)) {
			plxMsg::Error($plxPlugin->getLang('L_ERR_CACHE'));
			header('Location: plugin.php?p=plxPluginDownloader');
			exit;
		}

		# on teste si le fichier distant est dispo
		if(!plxPluginDownloader::is_RemoteFileExists($repo[$plugName]['file'])) {
		...
		if(!plxPluginDownloader::downloadRemoteFile($repo[$plugName]['file'], $zipfile)) {
		...
		require_once(PLX_PLUGINS."plxPluginDownloader/dUnzip2.inc.php");
...
if(!$remote_version = plxPluginDownloader::getRemoteFileContent($repository_url.$repository_version))
...
if($version=='' OR $version!=$remote_version or !is_file($cache_dir.$repository_xmlfile)) {
	plxPluginDownloader::downloadRemoteFile($repository_url.$repository_xmlfile, $cache_dir.$repository_xmlfile);
}
...
<form action="plugin.php?p=plxPluginDownloader" method="post" id="form_plugindownloader">
