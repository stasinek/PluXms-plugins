 ...
 * @author	Stephane F
 **/
class plxPrivateStatic extends plxPlugin {
	...
	public function AdminStatic() {
		echo '
			<?php
				...
				$filename = PLX_ROOT.$plxAdmin->aConf["racine_statiques"].$id.".plxPrivateStatic.php";
				...
				$image = "<img src=\"".PLX_PLUGINS."plxPrivateStatic/locker.png\" alt=\"\" />";
			?>
			...
			if(!empty($_POST)) {
				if(!empty($_POST["selection"]) AND $_POST["selection"]=="delete" AND isset($_POST["idStatic"])) {
					foreach($_POST["idStatic"] as $static_id) {
						$filename = PLX_ROOT.$plxAdmin->aConf["racine_statiques"].$static_id.".plxPrivateStatic.php";
    ...
    public function plxAdminEditStatique() {
		echo '<?php
			...
			if(!empty($content["chapo"]))
				plxUtils::write($content["chapo"], PLX_ROOT.$this->aConf["racine_statiques"].$content["id"].".plxPrivateStatic.php");
		?>';
    }
			...
			if(\$protect==1) {
				...
				if(\$password!='') {
					if(!isset(\$_SESSION['password_statics'][\$this->cible])) {
						...
						\$this->cible = '../../'.PLX_PLUGINS.'plxPrivateStatic/form';
	...
	public function ThemeEndHead() {
		echo "\t".'<link rel="stylesheet" type="text/css" href="'.PLX_PLUGINS.'plxPrivateStatic/style.css" media="screen" />'."\n";
	}
}
?>