<?php
/**
 * Plugin plxComRememberMe
 * @author	Stephane F
 **/
class plxComRememberMe extends plxPlugin {
	...
	public function plxMotorDemarrageNewCommentaire() {
		...
		if(isset($_POST["rememberme"]) AND ($retour[0]=="c" OR $retour=="mod")) {
			...
			if (version_compare(PHP_VERSION, "5.2.0", ">="))
				setcookie("plxComRememberMe", serialize($cookie_value), $cookie_expire, $cookie_path, $cookie_domain, $cookie_secure, true);
			else
	...
	public function plxMotorConstruct() {
		...
		$string = '
		if(isset($_COOKIE["plxComRememberMe"])) {
			$cookie_value = unserialize(plxUtils::unSlash($_COOKIE["plxComRememberMe"]));
