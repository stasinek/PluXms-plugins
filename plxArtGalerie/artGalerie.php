...
class artGalerie extends plxPlugin {
	...
	public function ArtgalerieDisplay($static_path) {
		...
		if ($plxShow->plxMotor->mode == 'article'){
			$galerie_path = (!empty($static_path))?$static_path :$plxShow->plxMotor->plxRecord_arts->f('galerie');
		}
		elseif($plxShow->plxMotor->mode == 'static' OR $plxShow->plxMotor->mode == 'home' OR $plxShow->plxMotor->mode == 'categorie' OR $plxShow->plxMotor->mode == 'archives' OR $plxShow->plxMotor->mode == 'tags') {
	...
	public function staticGalerieShow($static_path) {
		...
		if($plxShow->plxMotor->mode != 'static' or empty($static_path)) {
			return;
		}
		...
		if (count($dirs) > 0) {
			...
			foreach ($dirs as $dir){
				...
				$galeries[$i]['url'] = $plxShow->plxMotor->urlRewrite('?static'.intval($plxShow->plxMotor->cible).'/'.$plxShow->plxMotor->aStats[$plxShow->plxMotor->cible]['url'].'&galerie='.$path.'/'.$dir);
				if ($showThumb) {
					...
					$galeries[$i]['icon'] = $plxShow->plxMotor->urlRewrite($galeries[$i]['icon']);
				}
				else {
					$galeries[$i]['icon'] = $plxShow->plxMotor->urlRewrite(PLX_PLUGINS.'artGalerie/gallery-icon.png');
				}
				$i++;
			}
