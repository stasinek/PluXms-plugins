...
class plxCapchaImage extends plxPlugin {
	...
	public function plxShowCapchaQ() { //'.PLX_PLUGINS.'
		...
		$root = $plxMotor->urlRewrite(str_replace('./', '', PLX_PLUGINS).'plxCapchaImage/capcha.php');
		$_SESSION['capcha_token'] = sha1(uniqid(rand(), true));
		$_SESSION['capcha']=$this->getCode(5);
		echo '<img src="'.$root.'" alt="Capcha" id="capcha" />';
		echo '<a id="capcha-reload" href="javascript:void(0)" onclick="document.getElementById(\'capcha\').src=\''.$root.'?\' + Math.random(); return false;"><img src="'.PLX_PLUGINS.'plxCapchaImage/reload.png" title="" /></a><br />';
		$this->lang('L_MESSAGE');
